...
- **`spindll rm` no longer strips vision from a model you kept** — a vision repo ships one projector file shared by every quant variant it holds, and removing any single variant deleted it. The variants you kept stayed listed and loadable but silently lost image support, with nothing left on disk to recover from. The projector now survives for as long as any remaining model in its directory can use it — including the variants that record no path of their own and find it by scanning, which is how every model pulled as plain GGUF reaches it.
...
- **`spindll run <repo>` loads the variant `spindll pull` would have fetched** — for a repo holding several quants, a bare repo name resolved to whichever filename came first alphabetically. That puts `bf16` ahead of `q4_k_m`, so `run`, `load` and `chat` settled on the full-precision weight — research precision at 3–4× the size, and the one variant the download path itself would never pick. Resolution now applies the same quant preference as `pull`; name an exact variant to override it.
- **A projector that cannot be deleted no longer leaves a phantom entry behind** — when `rm` removed a model's weights but failed to remove its shared projector (a read-only directory, say), it gave up before saving the registry, which went on advertising a model whose file was already gone. The removal is recorded either way now, and an undeletable projector is reported as a warning instead.
